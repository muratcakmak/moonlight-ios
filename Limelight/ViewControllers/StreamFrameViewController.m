...
#else
    [_tipLabel setText:@"Tip: 3-finger tap for keyboard. Swipe from left edge to disconnect."];
#endif
...
- (void) connectionStarted {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        // OpenBench: Don't show gaming on-screen controls
...
- (void)connectionTerminated:(int)errorCode {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (portTestResults != ML_TEST_RESULT_INCONCLUSIVE && portTestResults != 0) {
            ...
            message = @"Your device's network connection is blocking OpenBench. Streaming may not work while connected to this network.";
        }
        else {
            switch (errorCode) {
